# src: https://github.com/allenai/allennlp/blob/master/allennlp/modules/maxout.py
...
class Maxout(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if len(output_dims) != num_layers:
            raise ValueError("len(output_dims) (%d) != num_layers (%d)" %
                             (len(output_dims), num_layers))
        if len(pool_sizes) != num_layers:
            raise ValueError("len(pool_sizes) (%d) != num_layers (%d)" %
